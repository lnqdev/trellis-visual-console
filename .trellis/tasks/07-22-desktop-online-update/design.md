...
`bundle.createUpdaterArtifacts=true`，每个版本必须准备：

- macOS arm64：DMG 手工安装包、`.app.tar.gz` 更新包及 `.sig`。
- macOS x64：DMG 手工安装包、`.app.tar.gz` 更新包及 `.sig`。
- Windows x64：NSIS 手工/更新安装包及 `.sig`。

Gitee Release 托管上述产物。仓库 `releases/latest.json` 使用 Tauri 静态清单格式，至少包含 `version`、非空中文 `notes`、`pub_date` 及 `darwin-aarch64`、`darwin-x86_64`、`windows-x86_64` 三个平台的 HTTPS URL 和签名。

发布顺序必须为：统一版本 -> 三平台原生构建 -> 校验签名/哈希 -> 上传同一 Gitee Release -> 验证匿名下载 -> 生成并校验清单 -> 最后更新 `main` 分支的 `releases/latest.json`。清单是唯一发布开关；任何平台缺失都不得更新。
...
- 前端 IPC mock 覆盖状态机与响应式布局；真实安装从 `0.2.0-beta.1` 升级到更高测试版本，验证三平台均不重复手工安装。
