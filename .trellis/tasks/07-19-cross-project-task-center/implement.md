...
- [ ] **步骤 1：在 `src/shared/api.ts` 定义任务中心合同**
...
- [ ] **步骤 2：在 `ProjectApiService` 聚合一次性任务响应**
...
- [ ] **步骤 3：在 `project-routes.ts` 注册无输入聚合路由**
...
- [ ] **步骤 3：创建 `src/web/hooks/useTaskCenter.ts` 的公开状态合同**
...
- [ ] **步骤 1：在 `useProjectConsole.ts` 增加顶层模式并组合 `useTaskCenter`**
...
- [ ] **步骤 3：将 SSE 改为稳定单连接和约 150ms 尾随防抖**
...
- [ ] **步骤 1：在 `ProjectSidebar.tsx` 增加全局入口**
...
- [ ] **步骤 2：创建 `src/web/components/TaskCenter.tsx`**
...
- [ ] **步骤 2：使用 `trellis-check` 和 `superpowers:requesting-code-review` 审查实现**
...
- [ ] 用户明确允许开始实施后，才运行 `task.py start` 和创建任务分支。
- [ ] 不创建 worktree，不新增测试类或仓库内独立测试文件。
- [ ] 实施前重新读取目录选择提交后所有冲突文件，禁止覆盖 `d96dc58` 已落地能力。
